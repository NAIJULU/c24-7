	...
	function setGoogleMarkersEstaciones(map)
	{
		...
		.done(function(data) {
			...
			$.each(data, function(i, estacion) {
				...
				var hashMarker 				= window.location.href;
				var hashMarker2 			= 0;
				...
				'</tr>'+
				'</table>'+
				'<div class="info-compartir">'+tweeterMarket+facebookMarket+'</div>'+
				'</div>'+
				'</article>';
...
function getGallery(event)
{
	...
	if(pag >= 1)
	{
		...
		jQuery.ajax({
			type: "GET",
			url: "../wp-admin/admin-ajax.php",
			data: {'action' : 'getGallery', 'paged':pag },
		})
		.done(function(data) {
			if(data == "")
			{
...
function getGalleryPerDate(event)
{
	...
	if( date.length )
	{
		...
		jQuery.ajax({
				type: "GET",
				url: "../wp-admin/admin-ajax.php",
				data: { 'action' : 'getGalleryPerDate', 'date' : date },
		})
		.done(function(data) {
			if(data == "")
			{
