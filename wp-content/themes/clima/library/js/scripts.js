...
f[a].setAttribute("aria-label",b);if(e[a].innerText)if(v)f[a].innerText=b;else{var m=document.createElement("p"),n=document.createTextNode(b);m.appendChild(n);m.setAttribute("aria-label",b);f[a]=m}else f[a].textContent=b}b="";s?(r&&(b+=''),b+='<p class="tweet">'+x(e[a].innerHTML)+"<i class='icon-asterisk icon-white'></i></p>",q&&(b+='')):e[a].innerText?(r&&(b+=''),b+='<p class="tweet">'+e[a].innerText+
